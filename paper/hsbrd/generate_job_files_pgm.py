...
# E. coli dist: lq med uq quartiles of crowders
mu_sigma = [(31.9, 0.825), (12.1, 0), (21.1, 0),(36.8, 0) ]
realizations = 10
...
# Cellular concetration of g3p approx 49 muM
conc_g3p = 49e-6
conc_pgm = 64e-6


substrate_concentrations = np.array([0.25, 0.5, 1., 2.0, 4.0 ])*conc_g3p
product_concentrations =   np.array([0.25, 0.5, 1., 2.0, 4.0 ])*conc_g3p
enzyme_concentrations = np.array([0.25, 0.5, 1., 2.0, 4.0 ])*conc_pgm
enzyme_complex_concentrations =  np.array([0.25, 0.5, 1., 2.0, 4.0 ])*conc_pgm
...
input_file = "/scratch/weilandt/src/pmes/papers/geek_crowding_1st/cluster/pgm_cluster.py"
folder =  "./input_case_study_equilibrium_"+timestamp+"/"
log_folder = "/scratch/weilandt/data/multiscale_paper_2017/out_"+timestamp
os.mkdir(log_folder)
os.mkdir(folder)

output = "/scratch/weilandt/data/multiscale_paper_2017/full_factorial_pgm_equilibrium_dw_10_10_2018/run_"+timestamp
...
try:
   for this_mu, thi_sigma in mu_sigma:
      for this_volume_fraction in volume_fractions:
         # Do simulate 0 volume fraction only once
         if thi_sigma == 0.0 and this_volume_fraction == 0.0:
            continue

         for this_substrate_concentration in substrate_concentrations:
             for this_product_concentration in product_concentrations:
                 for this_pgm_free in enzyme_concentrations:
                     for this_pgm_complex in enzyme_complex_concentrations:
                         ...
                         for this_realization in range(realizations):
                            ...
                            if this_substrate_concentration > 0:
                               this_job["S_concentration"]  = round_to_e5(this_substrate_concentration)
                            else:
                               this_job["S_concentration"]  = this_substrate_concentration

                            if this_substrate_concentration > 0:
                               this_job["P_concentration"]  = round_to_e5(this_product_concentration)
                            else:
                               this_job["P_concentration"]  = this_product_concentration

                            if this_substrate_concentration > 0:
                               this_job["E_concentration"]  = round_to_e5(this_pgm_free)
                            else:
                               this_job["E_concentration"]  = this_pgm_free

                            if this_substrate_concentration > 0:
                               this_job["ES_concentration"]  = round_to_e5(this_pgm_complex)
                            else:
                               this_job["ES_concentration"] = this_pgm_complex
                            ...
                            if counter >= chunksize:
                               ...
                               cpu_time = cpu_time_from_particles(max(n_particles))
...
def write_sbatch_file(data_frame,input_file,folder,this_id,cpu_time):
   ...
   file_obj.write("#SBATCH --workdir /scratch/weilandt/data/multiscale_paper_2017/out_"+timestamp+"\n")
   ...
   file_obj.write("\n")
   file_obj.write("source /scratch/weilandt/src/env_pymes/bin/activate \n")
